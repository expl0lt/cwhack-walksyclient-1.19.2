...
public enum CrystalUtils
{
    ...
    public static boolean canPlaceCrystalServer(BlockPos block)
    {
        ...
        List<Entity> list = WalksyClient.getClient().world.getOtherEntities((Entity)null, new Box(d, e, f, d + 1.0D, e + 2.0D, f + 1.0D));
        ...
        list.removeIf(entity ->
        {
            if (!(entity instanceof EndCrystalEntity))
                return false;
            return WalksyClient.getInstance().getCrystalDataTracker().isCrystalAttacked(entity);
    ...
    public static boolean canPlaceCrystalClientAssumeObsidian(BlockPos block, Box bb)
    {
        ...
        if (crystalBox.intersects(bb))
            return false;
        List<Entity> list = WalksyClient.getClient().world.getOtherEntities((Entity)null, crystalBox);
        return list.isEmpty();
    }
    ...
    public static double crystalDamage(PlayerEntity player, Vec3d playerPos, Vec3d crystal, BlockPos obsidianPos, boolean ignoreTerrain) {
        ...
        Explosion explosion = new Explosion(WalksyClient.getClient().world, null, crystal.x, crystal.y, crystal.z, 6);
    ...
    private static double getDamageForDifficulty(double damage) {
        return switch (WalksyClient.getClient().world.getDifficulty()) {
            case PEACEFUL -> 0;
    ...
    private static BlockHitResult raycast(RaycastContext context, BlockPos obsidianPos, boolean ignoreTerrain) {
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockPos) -> {
            ...
            if (blockPos.equals(obsidianPos)) blockState = Blocks.OBSIDIAN.getDefaultState();
                ...
                blockState = WalksyClient.getClient().world.getBlockState(blockPos);
            ...
            VoxelShape voxelShape = raycastContext.getBlockShape(blockState, WalksyClient.getClient().world, blockPos);
            BlockHitResult blockHitResult = WalksyClient.getClient().world.raycastBlock(vec3d, vec3d2, blockPos, voxelShape, blockState);
