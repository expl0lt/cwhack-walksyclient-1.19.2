package net.walksy.client.misc.attributes;
...
import net.walksy.client.misc.TextRenderer;
...
public class VanillaTextRenderer implements TextRenderer {
    ...
    public double getWidth(String text, int length, boolean shadow) {
        ...
        return (WalksyClient.getClient().textRenderer.getWidth(text) + (shadow ? 1 : 0)) * scale;
    }

    @Override
    public double getHeight(boolean shadow) {
        return (WalksyClient.getClient().textRenderer.fontHeight + (shadow ? 1 : 0)) * scale;
    }
    ...
    public double render(String text, double x, double y, Color color, boolean shadow) {
        ...
        double x2 = WalksyClient.getClient().textRenderer.draw(text, (float) (x / scale), (float) (y / scale), color.getPacked(), shadow, matrix, immediate, false, 0, LightmapTextureManager.MAX_LIGHT_COORDINATE);
