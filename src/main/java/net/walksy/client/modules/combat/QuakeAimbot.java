package net.walksy.client.modules.combat;
...
import net.walksy.client.components.systems.TargetCircle;
...
public class QuakeAimbot extends Module {
    ...
    */
    private List<Entity> getTargets() {
        ...
        Vec3d localPos = WalksyClient.me().getPos();
        int localTeamColour = this.getTeamColour(WalksyClient.me());

        // Get all of the players
        for (Entity ent : WalksyClient.getClient().world.getEntities()) {
            if (!(ent instanceof PlayerEntity)) continue;

            Entity player = ent;

            // Make sure the player is not us
            if (player == WalksyClient.me()) continue;
            ...
            boolean canSee = this.getBoolSetting("Preaim") ? ClientUtils.canSee(pos) : WalksyClient.me().canSee(player);
            if (!canSee) continue;
    ...
    private Entity getTarget() {
        ...
        RotationUtils.Rotation current = new RotationUtils.Rotation(WalksyClient.me().getYaw(), WalksyClient.me().getPitch());
        ...
        players.sort(new Comparator<Entity>() {
            ...
            public int compare(Entity a, Entity b) {
                ...
                RotationUtils.Rotation aRotation = RotationUtils.getRequiredRotation(aPos);
                RotationUtils.Rotation bRotation = RotationUtils.getRequiredRotation(bPos);
    ...
    private void aimbotThink(float tickDelta) {
        ...
        RotationUtils.Rotation targetRotation = RotationUtils.getRequiredRotation(localPos, targetPos, tickDelta);
        RotationUtils.Rotation current = new RotationUtils.Rotation(WalksyClient.me().getYaw(), WalksyClient.me().getPitch());
        RotationUtils.Rotation diff = targetRotation.difference(current);
        ...
        WalksyClient.me().setPitch(pitch);
        WalksyClient.me().setYaw(yaw);
        ...
        if (this.getBoolSetting("AutoShoot")) {
            ...
            // Shooting delay
            double curr = WalksyClient.getCurrentTime();
