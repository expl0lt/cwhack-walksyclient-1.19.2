...
public class EntityOwner extends Module {
    ...
    private void renderName(String name, Vec3d pos, float tickDelta, MatrixStack mStack) {
		...
		mStack.multiply(WalksyClient.getClient().getEntityRenderDispatcher().getRotation());
		float c = (float)Math.sqrt(WalksyClient.getClient().cameraEntity.getLerpedPos(tickDelta).distanceTo(pos));
