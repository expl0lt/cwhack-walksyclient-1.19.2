...
import net.walksy.client.components.systems.binds.impl.CommandBind;
...
import net.walksy.client.utils.ThirdPartyUtils;
...
public class Binds extends Module {
    ...
    private Boolean isChatDelimiter(Integer key) {
        ...
        return (keyChar == WalksyClient.getInstance().commandHandler.delimiter.charAt(0));
    }
    ...
    public void fireEvent(Event event) {
        switch (event.getClass().getSimpleName()) {
            case "OnKeyEvent": {
                ...
                // Handle the keypress
                if (WalksyClient.getClient().currentScreen == null) binds.fireBinds(e.key);

                // Open chat if it is our command button.
                if (this.isChatDelimiter(e.key) && WalksyClient.getClient().currentScreen == null) {
                    ClientUtils.openChatScreen();
                }

                // Open ClickGUI (if we dont have meteor)
                if (!ThirdPartyUtils.isOtherClientLoaded() && this.getBoolSetting("GUIKey") && e.key == WalksyClient.getInstance().config.menuKey) {
                    ChatUtils.hideNextChat = true;
                    WalksyClient.getInstance().getModules().get("clickgui").toggle();
                }
        ...
        if (data.containsKey("binds")) {
            ...
            for (String key : bindsObj.keySet()) {
                ...
                for (String command : commands) {
                    ...
                    if (WalksyClient.getInstance().getModules().containsKey(command)) {
                        Module module = WalksyClient.getInstance().getModules().get(command);
    ...
    private static class BindsSub extends Command {
        ...
        protected Module getModule(String command) {
            WalksyClient cc = WalksyClient.getInstance();
